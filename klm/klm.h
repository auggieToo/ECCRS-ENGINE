...
#define LITLIST(...)                                                           \
  ((litList){(lit[]){__VA_ARGS__}, sizeof((lit[]){__VA_ARGS__}) / sizeof(lit)})

#define THEN LITLIST
#define IF LITLIST

#define DEFEASIBLE_RULE(kb, head, body)                                        \
...
ruleId kbAddRuleWithName(knowledgeBase *kb, ruleType type,
                         ...
                         const literalType *bSigns, u32 bCount);

ruleId kbAddRuleLits(knowledgeBase *kb, ruleType type, litList head,
                     litList body);
